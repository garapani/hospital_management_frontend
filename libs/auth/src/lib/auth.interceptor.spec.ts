...
describe('authInterceptor', () => {
  ...
  it('on a 401, refreshes once and retries the original request with the new token', () => {
    ...
    http.get('https://gateway.example/api/invoices').subscribe((res) => (result = res));
    ...
    const refresh = httpMock.expectOne('https://gateway.example/api/auth/refresh');
    refresh.flush({ accessToken: newAccessToken, refreshToken: 'refresh-token-2' });
  ...
  it('gives up after one retry if the retried request also 401s, clearing the session', () => {
    ...
    http.get('https://gateway.example/api/invoices').subscribe({ error: (err) => (errored = err) });
    ...
    httpMock
      .expectOne('https://gateway.example/api/invoices')
      .flush({ message: 'still expired' }, { status: 401, statusText: 'Unauthorized' });
  ...
  it('does not treat a protected endpoint containing "/auth/login" as a substring as the login endpoint', () => {
    ...
    http.get('https://gateway.example/api/audit-log/auth/login-events').subscribe();

    const req = httpMock.expectOne('https://gateway.example/api/audit-log/auth/login-events');
    expect(req.request.headers.get('Authorization')).toBe(`Bearer ${authService.getAccessToken()}`);
    req.flush({});
  });
});
